...
class URLPipeline:
    ...
    def process_item(self, item, spider):
        if spider.name == 'URL':
            ...
            tmp = {
                'num': int(item['num']), 'code': item['code'], 'title': item['title'], 'url': item['url']
            }
    ...
    def process_item(self, item, spider):
        if spider.name == 'SET':
            ...
            for each in self.items[int(item['num']) - 1]:
                ...
                index += 1
            tmp = {
                'title': item['title'], 'url_list': item['url_list'], 'src_link': item['src_link'],
...
class SRCPipeline:
    def open_spider(self, spider):
        if spider.name == 'SRC':
            ...
            for index in range(6):
                tmp = {'src_id': 0, 'src_link': [], 'caption': [], 'option': [], 'answer':[]}
                sets.append(tmp)
            section = []
            for index in range(6):
    ...
    def process_item(self, item, spider):
        if spider.name == 'SRC':
            ...
            self.tmp_list[set_num][index - 1][id_num]['src_link'] = item['src_link']
        ...
        if spider.name == 'SRC':
            for section in range(69):
                if section > 57 and section < 62:
                    continue
                for sets in range(6):
                    if section > 53 and sets == 5:
                        break
                    name = 'cache/src/' + str(section + 1) + '-' + str(sets + 1) + '.json'
                    with open(name, 'w', encoding='utf-8') as file:
                        tmp = self.tmp_list[section][sets]
                        if tmp[5]['src_id'] == 0:
                            file.write(json.dumps(tmp[:5], indent=4, ensure_ascii=False))
                        else:
                            file.write(json.dumps(tmp, indent=4, ensure_ascii=False))
